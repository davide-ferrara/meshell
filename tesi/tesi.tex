...
\begin{abstract}
    ...
    \begin{figure}[h!]
        ...
        % \caption{MeShell Logo}
        \label{fig:Meshell Logo}
    \end{figure}
...
Il seguente script creerá un link simbolico in /usr/bin che permetterá di eseguire i vari comandi Meshell all'interno dei client.
\begin{lstlisting}{go}
...
Da questo output del comando pstree possiamo vedere come Alacritty (l'emulatore), abbia come figli Bash e pstree che é il comando eseguito.
...
In seguito le principali operazioni del \textit{tty hanlder:}
\begin{enumerate}
...
Nel caso vada tutto a buon fine la funzione ritorna un handle alla connessione SSH di una specifica macchina.
...
(\texttt{"xterm"}) e le dimensioni.
Xterm è l'emulatore standard di terminale in ambiente Unix-like.
Un utente può avere più sessioni di xterm avviate su uno o più display, le quali forniscono un sistema di input/output per i processi lanciati.
Senza un PTY, non sarebbe possibile avere una shell interattiva, ma solo eseguire comandi singoli. Subito dopo,
\texttt{session.Shell()} avvia la shell remota associata a questo PTY.

\subsection{Flusso Dati Bidirezionale (startPiping)}
Questa funzione orchestra il flusso di dati tra il client web e la shell remota tramite tre \textbf{goroutine} concorrenti.
\begin{lstlisting}[language=Go]
...
rapidi offrono scorciatoie per le directory \texttt{..} (parent), \texttt{\~}
(home) e \texttt{/} (root).
\begin{lstlisting}{go}
"cd")
...
\paragraph{w} Mostra chi é attualmente loggato nel sistema e cosa sta facendo.
\begin{lstlisting}{go}
"w")
...
\paragraph{which} Mostra il percorso completo di un comando.
\begin{lstlisting}{go}
"which")
...
\paragraph{type} Mostra il tipo di un comando (es. alias, builtin, file).
\begin{lstlisting}{go}
    echo -n "Inserisci il nome del comando: "
    read command
...
\paragraph{alias} Crea un alias per un comando. L'alias viene salvato in bash\_aliases.
\begin{lstlisting}{go}
...
\paragraph{Rimuovi Alias} Apre il file \texttt{.bash\_aliases} con \texttt{vi} per rimuovere un alias.
\begin{lstlisting}{go}
"alias-remove")
    vi ~/.bash_aliases
    ;;
...
\paragraph{prompt} Mostra il prompt della shell corrente.
\begin{lstlisting}{go}
"prompt")
...
\paragraph{path} Mostra il percorso dal quale la shell cerca gli eseguibili.
\begin{lstlisting}{go}
"path")
...
\paragraph{bashrc} Apre il file .bashrc con \texttt{vi} per modificare la configurazione della shell.
\begin{lstlisting}{go}
"bashrc")
    vi ~/.bashrc
    ;;
...
\paragraph{lshw} Mostra le informazioni sull'hardware del sistema.
\begin{lstlisting}{go}
"lshw")
...
\paragraph{lsblk} Mostra i dispositivi a blocchi.
\begin{lstlisting}{go}
"lsblk")
...
\paragraph{lspci} Mostra i dispositivi PCI.
\begin{lstlisting}{go}
"lspci")
...
\paragraph{lsusb} Mostra i dispositivi USB.
\begin{lstlisting}{go}
"lsusb")
...
"dmesg")
    ...
    ;;
\end{lstlisting}
\paragraph{journalctl} Mostra il journal di sistema.
\begin{lstlisting}{go}
"journalctl")
...
Il backend Go gestisce la complessità del protocollo SSH e del PTY mentre il frontend JavaScript con
Xterm.js offre un'esperienza utente fluida e nativa con la poossibilitá di amministrare piú macchine da una
singola pagina web grazie allo script Bash che astrae i comandi, rendendoli accessibili e riducendo la possibilità di errori.
\textit{La seguente tesi fornisce un approossimazione dei vari comandi e la loro implementazione, per ulteriori dettagli
    é consigliato leggere il codice sorgente.}
\end{document}
